...
# All constants from dict.py (OBJECT_TYPE, CREATE_OBJECT_TYPE, etc.)
...
# This indicates that this route handles creation operations
module = "create"
...
# 2. object_type - Required select field that dynamically loads available object types for creation
base_schema = {
    "metadata": {},
    "fields": [
        ...
        # This field dynamically loads available object types that can be created
        {
            "type": "string",
            ...
            "choices": {
                "values": [
                    {"value": obj_type} for obj_type in CREATE_OBJECT_TYPE
                ]
            },
            "validation": {
                "required": True
...
# It returns different schemas based on the selected object type for creation
...
def schema():
    ...
    # Extract required and optional fields for the 'create' action
    ...
    # These fields must be filled out for the creation to succeed
...
# Route handler for executing Stripe API creation calls
# This endpoint processes the form submission and creates new Stripe objects
...
def execute():
    ...
    # Debug logging of the final data to be sent to Stripe
    print(data)
    ...
    # Each object type has its own specific creation logic
    if object_type == "customer":
        # Create a new customer
        # All customer fields are passed directly to the API
        customer = stripe.Customer.create(**data)
        return Response(data=customer)

    elif object_type == "charge":
        # Create a new charge
        # Special handling for source/customer requirement
        source = data.get("customer") or data.get("source")
        if not source:
            return Response("Please provide either a source or a customer ID")
        charge = stripe.Charge.create(**data)
        return Response(data=charge)

    elif object_type == "payment_intent":
        # Create a new payment intent
        # All payment intent fields are passed directly to the API
        payment_intent = stripe.PaymentIntent.create(**data)
        return Response(data=payment_intent)

    # Return the data if no object type matches
