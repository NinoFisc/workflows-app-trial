...
base_schema = {
    ...
    "fields": [
        ...
        # Object Type selector field definition
        {
            ...
            "choices": {
                "values": [
                    {"value": obj_type} for obj_type in GET_OBJECT_TYPE
                ]
            },
            "validation": {
                "required": True
...
def execute():
    ...
    if object_type == "customer":
        ...
        id = data.get("customer")
        if not id:
            return Response(data="Please provide a customer ID")
        ...
        id = data.get("charge")
        if not id:
            return Response(data="Please provide a charge ID")
    ...
    elif object_type == "balance":
        # Retrieve balance (no ID required)
        balance = stripe.Balance.retrieve()
        return Response(data=balance)

    elif object_type == "balance_transaction":
        # Retrieve balance transaction by ID
        id = data.get("balance_transaction")
        if not id:
            return Response(data="Please provide a balance transaction ID")

        balance_transaction = stripe.BalanceTransaction.retrieve(id)
        return Response(data=balance_transaction)
